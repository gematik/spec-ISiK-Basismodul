...
* parameter[+]
  ...
  * min = 0
  * max = "1"
  * documentation	= "Falls die Interaktion fehlschlägt, MUSS als Return-Parameter eine OperationOutcome-Ressource zurückgegeben werden. Syntaktisch fehlerhafte Appointment-Ressourcen (z. B. ungültige XML-/JSON-Repräsentationen gemäß Kernspezifikation) MÜSSEN mit dem HTTP Status-Code 400 – Bad Request abgewiesen werden. Semantisch invalide Appointment-Ressourcen (z.B. bei inkorrekter semantischer Kodierung der Fachrichtung (serviceType) oder bei Verstößen gegen Kardinalitäten gemäß dem ISiKTermin-Profil) MÜSSEN mit einer OperationOutcome-Ressource abgewiesen werden. In diesem Fall SOLL der HTTP Status-Code HTTP 422 - Unprocessable Entity verwendet werden. Eine OperationOutcome-Ressource KANN zu informativen Zwecken in jedem Fall zurückgegeben werden (s.o.)."
