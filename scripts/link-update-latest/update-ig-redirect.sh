...
log_info "Discovering ISiK modules…"
...
while IFS= read -r MODULE; do
  ...
  if [[ "$VERSION" == "latest" ]]; then
    ...
    while IFS= read -r FOLDER; do
      if is_version_folder "$FOLDER"; then
        TARGET_VERSION="$FOLDER"
      fi
    done < <(echo "$VERSIONED" | sort -V)
  ...
  if [[ -z "$HTML_LIST" ]]; then
    log_info "${MODULE}: no latest/ folder (or no HTML files), skipping."
    continue
  fi
  ...
  log_ok "${MODULE}: updated ${FILE_COUNT} file(s) → ${TARGET_VERSION}"
